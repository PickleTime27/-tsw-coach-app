...
const SYMPTOMS = [
  { key: "itching", label: "Itching", emoji: String.fromCodePoint(0x1F525), description: "Skin irritation & itch intensity" },
  { key: "redness", label: "Redness", emoji: String.fromCodePoint(0x1F7E5), description: "Skin redness & inflammation" },
  { key: "flaking", label: "Flaking", emoji: String.fromCodePoint(0x1FAE7), description: "Skin shedding & flaking" },
  { key: "oozing", label: "Oozing", emoji: String.fromCodePoint(0x1F4A7), description: "Weeping or oozing areas" },
  { key: "swelling", label: "Swelling", emoji: String.fromCodePoint(0x1F388), description: "Puffiness or swelling" },
  { key: "sleep_quality", label: "Sleep Quality", emoji: String.fromCodePoint(0x1F319), description: "10 = great sleep, 0 = no sleep" },
  { key: "pain", label: "Pain", emoji: String.fromCodePoint(0x26A1), description: "Skin pain or burning" },
  { key: "dryness", label: "Dryness", emoji: String.fromCodePoint(0x1F3DC), description: "Skin dryness & tightness" },
  { key: "nerve_pain", label: "Nerve Pain", emoji: String.fromCodePoint(0x1F9E0), description: "Tingling, burning, or nerve sensations" },
  { key: "thermal_regulation", label: "Thermal Regulation", emoji: String.fromCodePoint(0x1F321), description: "Hot flashes, chills, or temperature swings" },
];
...
export default function SymptomTracker() {
  ...
  useEffect(() => {
    ...
    if (!profile || !profile.profileId) { router.push("/onboarding"); return; }
    fetch(`/api/symptoms?profileId=${profile.profileId}&days=1`)
      .then(r => r.json())
      .then(data => {
        if (data.logs && data.logs.length > 0) {
          const today = new Date().toDateString();
          const hasToday = data.logs.some((l: { logged_at: string }) => new Date(l.logged_at).toDateString() === today);
          if (hasToday) setTodayLogged(true);
        }
      }).catch(() => {});
  }, [router]);

  const overallScore = Math.round(Object.values(values).reduce((sum, v) => sum + v, 0) / SYMPTOMS.length);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("/api/symptoms", {
        ...
        body: JSON.stringify({ profileId: profile.profileId, ...values, overall_score: overallScore, notes }),
      });
      if (res.ok) { setSaved(true); setTodayLogged(true); setTimeout(() => setSaved(false), 3000); }
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: WARM_CREAM, color: DEEP_FOREST, minHeight: "100vh" }}>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input[type="range"]::-webkit-slider-thumb { -webkit-appearance: none; width: 24px; height: 24px; border-radius: 50%; background: white; border: 3px solid #1B6B4A; cursor: pointer; box-shadow: 0 2px 6px rgba(0,0,0,0.15); }
      ...
      <div style={{ padding: "16px 24px", borderBottom: "1px solid rgba(27,107,74,0.1)", background: "rgba(253,248,240,0.95)", backdropFilter: "blur(20px)", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 36, height: 36, borderRadius: "50%", background: BALM_GREEN, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ color: "white", fontSize: 18 }}>{String.fromCodePoint(0x1F4CA)}</span>
          </div>
          <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={() => router.push("/progress")} style={{ padding: "8px 16px", borderRadius: 8, border: `1px solid ${BALM_GREEN}`, background: "transparent", color: BALM_GREEN, fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>{String.fromCodePoint(0x1F4C8)} Progress</button>
          <button onClick={() => setShowMenu(!showMenu)} style={{ padding: "8px 12px", background: "transparent", border: "1px solid rgba(27,107,74,0.2)", borderRadius: 8, cursor: "pointer", fontSize: 18 }}>{String.fromCodePoint(0x2630)}</button>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "24px 20px 120px" }}>
        {todayLogged && <div style={{ background: SAGE_LIGHT, borderRadius: 16, padding: "16px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12, border: "1px solid rgba(27,107,74,0.15)" }}>
          <span className="check-pop" style={{ fontSize: 24 }}>{String.fromCodePoint(0x2705)}</span>
        ...
        <div style={{ background: "white", borderRadius: 16, padding: "20px 24px", marginTop: 16, boxShadow: "0 2px 8px rgba(13,40,24,0.04)", border: "1px solid rgba(27,107,74,0.06)" }}>
          <div style={{ fontWeight: 600, fontSize: 15, color: DEEP_FOREST, marginBottom: 10 }}>{String.fromCodePoint(0x1F4DD)} Notes (optional)</div>
        ...
        <button onClick={handleSave} disabled={saving} style={{ width: "100%", padding: "18px", borderRadius: 16, border: "none", fontWeight: 700, fontSize: 16, fontFamily: "'DM Sans', sans-serif", cursor: saving ? "not-allowed" : "pointer", marginTop: 24, background: saved ? "#4CAF50" : BALM_GREEN, color: "white", transition: "all 0.3s ease", boxShadow: "0 4px 16px rgba(27,107,74,0.25)" }}>
          {saving ? "Saving..." : saved ? String.fromCodePoint(0x2713) + " Saved!" : todayLogged ? "Update Today's Log" : "Save Today's Log"}
        </button>
