...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    supabase.from("profiles").select("id").eq("first_name", p.firstName).limit(1).then(function(res) {
      if (res.data && res.data[0]) {
        ...
        setMyId(pid);
        supabase.from("notification_prefs").select("*").eq("profile_id", pid).limit(1).then(function(r) {
          if (r.data && r.data[0]) {
            setCheckin(r.data[0].checkin_frequency);
            setResearch(r.data[0].research_alerts);
            setNewMember(r.data[0].new_member_alerts);
            setFriendReq(r.data[0].friend_request_alerts);
            setDm(r.data[0].dm_alerts);
          }
        });
      }
    });
  ...
  const savePrefs = async () => {
    if (!myId) return;
    await supabase.from("notification_prefs").upsert({ profile_id: myId, checkin_frequency: checkin, research_alerts: research, new_member_alerts: newMember, friend_request_alerts: friendReq, dm_alerts: dm });
    setSaved(true);
