...
export default function AdminPage() {
  ...
  if (authed === null) return <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C, color: G }}>Loading...</div>;
  if (!authed) return (
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C, fontFamily: "sans-serif" }}>
      <div style={{ background: "white", borderRadius: 20, padding: "48px 40px", maxWidth: 400, width: "90%", boxShadow: "0 8px 40px rgba(0,0,0,0.1)" }}>
        ...
        <form onSubmit={handleLogin}>
          ...
          <button type="submit" disabled={loading || !password} style={{ width: "100%", padding: "14px", background: loading ? "#B0D4B8" : G, color: "white", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: "pointer" }}>{loading ? "Verifying..." : "Access Dashboard"}</button>
        </form>
        <button onClick={() => router.push("/chat")} style={{ width: "100%", marginTop: 12, padding: "12px", background: "transparent", border: "none", color: "#5BA68A", fontSize: 14, cursor: "pointer" }}>Back to BALM</button>
      </div>
    </div>
  );
  return (
    <div style={{ minHeight: "100vh", background: C, fontFamily: "sans-serif" }}>
      ...
      <div style={{ padding: 32, maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ background: "white", borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" as any, fontSize: 14 }}>
            <thead><tr style={{ borderBottom: "2px solid #E8F5EC" }}>
              {["Name", "Role", "Admin", "Joined"].map(h => <th key={h} style={{ textAlign: "left" as any, padding: "10px 12px", color: "#5BA68A", fontWeight: 600 }}>{h}</th>)}
            </tr></thead>
            <tbody>{stats?.users.map((u: any) => (
